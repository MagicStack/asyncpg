...
from asyncpg import connect_utils, connection, exceptions
...
class TestSettings(tb.ConnectedTestCase):
    async def test_get_settings_01(self):
        self.assertEqual(self.con.get_settings().client_encoding, "UTF8")

    async def test_server_version_01(self):
        version = self.con.get_server_version()
        version_num = await self.con.fetchval(
            "SELECT current_setting($1)", "server_version_num", column=0
        )
...
class TestAuthentication(tb.ConnectedTestCase):
    def setUp(self):
        ...
        for method, password in methods:
            ...
            if method == "scram-sha-256":
                create_script.append("SET password_encryption = 'scram-sha-256';")

            create_script.append(
                "CREATE ROLE {}_user WITH LOGIN{};".format(
                    username, " PASSWORD {!r}".format(password) if password else ""
                )
            )
    ...
    async def test_auth_bad_user(self):
        with self.assertRaises(asyncpg.InvalidAuthorizationSpecificationError):
    ...
    async def test_auth_password_cleartext(self):
        conn = await self.connect(user="password_user", password="correctpassword")
        await conn.close()
        ...
        with self.assertRaisesRegex(
            ...
            await self._try_connect(user="password_user", password="wrongpassword")
    ...
    async def test_auth_password_cleartext_callable(self):
        ...
        with self.assertRaisesRegex(
            ...
            await self._try_connect(user="password_user", password=get_wrongpassword)
    ...
    async def test_auth_password_md5(self):
        ...
        with self.assertRaisesRegex(
            ...
            await self._try_connect(user="md5_user", password="wrongpassword")
    ...
    async def test_auth_password_scram_sha_256(self):
        ...
        conn = await self.connect(user="scram_sha_256_user", password="correctpassword")
        await conn.close()
        ...
        with self.assertRaisesRegex(
            ...
            await self._try_connect(user="scram_sha_256_user", password="wrongpassword")
        ...
        for password in passwords:
            ...
            conn = await self.connect(user="scram_sha_256_user", password=password)
            await conn.close()

        alter_password = "ALTER ROLE scram_sha_256_user PASSWORD 'correctpassword';"
...
class TestConnectParams(tb.TestCase):
    ...
    TESTS = [
        ...
        },
        {
            ...
            "result": (
                [("host2", 456)],
                {"user": "user2", "password": "passw2", "database": "db2"},
            ),
        },
        {
        ...
        },
        {
            ...
            "result": (
                [("localhost", 5555)],
                {"user": "user3", "password": "123123", "database": "abcdef"},
            ),
        },
        {
        ...
        },
        {
            ...
            "result": (
                [os.path.join("/unix_sock/test", ".s.PGSQL.5432")],
                {"user": "spam", "database": "dbname"},
            ),
        },
        {
        ...
        },
        {
            ...
            "result": (
                [
                    os.path.join(
                        "/cloudsql/project:region:instance-name", ".s.PGSQL.5432"
                    )
                ],
                {"user": "spam", "database": "db"},
            ),
        },
        {
    ...
    def test_test_connect_params_environ(self):
        ...
        try:
            ...
            with self.environ(AAAAAAAAAA123="1", AAAAAAAAAA456="2", AAAAAAAAAA789=None):
    ...
    def test_connect_pgpass_regular(self):
        ...
        try:
            ...
            self.run_testcase(
                {
                    "dsn": "postgres://user@abc/db?passfile={}".format(passfile.name),
                    "result": (
                        [("abc", 5432)],
                        {
                            "password": "password from pgpass for user@abc",
    ...
    def test_connect_pgpass_badness_mode(self):
        ...
        with tempfile.NamedTemporaryFile("w+t") as passfile:
            os.chmod(
                passfile.name, stat.S_IWUSR | stat.S_IRUSR | stat.S_IWGRP | stat.S_IRGRP
            )
...
class TestConnection(tb.ConnectedTestCase):
    ...
    async def test_connection_use_after_close(self):
        ...
        for meth in ("execute", "fetch", "fetchval", "fetchrow", "prepare", "cursor"):
    ...
    async def test_connection_ssl_to_no_ssl_server(self):
        ...
        with self.assertRaisesRegex(ConnectionError, "rejected SSL"):
            await self.connect(host="localhost", user="ssl_user", ssl=ssl_context)
        ...
        async def verify_works(sslmode):
            ...
            try:
                con = await self.connect(
                    dsn="postgresql://foo/?sslmode=" + sslmode, host="localhost"
                )
...
class BaseTestSSLConnection(tb.ConnectedTestCase):
    ...
    def setup_cluster(cls):
        ...
        cls.start_cluster(cls.cluster, server_settings=cls.get_server_settings())
...
class TestSSLConnection(BaseTestSSLConnection):
    ...
    async def test_ssl_connection_custom_context(self):
        ...
        con = await self.connect(host="localhost", user="ssl_user", ssl=ssl_context)
    ...
    async def test_ssl_connection_sslmode(self):
        ...
        async def verify_fails(sslmode, *, host="localhost", exn_type=ssl.SSLError):
            # XXX: uvloop artifact
    ...
    async def test_ssl_connection_default_context(self):
        ...
        try:
            ...
            with self.assertRaisesRegex(ssl.SSLError, "verify failed"):
                await self.connect(host="localhost", user="ssl_user", ssl=True)
        finally:
            self.loop.set_exception_handler(old_handler)
    ...
    async def test_executemany_uvloop_ssl_issue_700(self):
        ...
        try:
            ...
            await con.executemany(
                "INSERT INTO test_many VALUES ($1)", [(x + 1,) for x in range(100)]
            )
            self.assertEqual(await con.fetchval("SELECT sum(v) FROM test_many"), 5050)
        finally:
            try:
...
class TestNoSSLConnection(BaseTestSSLConnection):
    ...
    async def test_nossl_connection_prefer_cancel(self):
        con = await self.connect(
            dsn="postgresql://foo/?sslmode=prefer", host="localhost", user="ssl_user"
        )
        self.assertFalse(con._protocol.is_ssl)
        with self.assertRaises(asyncio.TimeoutError):
            await con.execute("SELECT pg_sleep(5)", timeout=0.5)
...
class TestConnectionGC(tb.ClusterTestCase):
    ...
    async def test_no_explicit_close_no_debug(self):
        ...
        try:
            with self.assertWarnsRegex(
                ResourceWarning, r"unclosed connection.*run in asyncio debug"
            ):
    ...
    async def test_no_explicit_close_with_debug(self):
        ...
        try:
            with self.assertWarnsRegex(ResourceWarning, r"unclosed connection") as rw:
                await self._run_no_explicit_close_test()
